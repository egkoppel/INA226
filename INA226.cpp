...
*******************************************************************************************************************/
uint8_t INA226_Class::begin(const uint64_t current_LSB,                         // Class initializer                //
                            const uint64_t calibration,                         //                                  //
  ...
  ina.current_LSB = current_LSB;                    // Get the best possible LSB in nA  //
  ina.calibration = calibration;                  // using 64 bit numbers throughout  //
  ina.power_LSB   = 25*ina.current_LSB*0.001;                             // Fixed multiplier for INA219      //
...
uint16_t INA226_Class::getBusMilliVolts(const bool waitSwitch,                //                                  //
  ...
  busVoltage = (uint32_t)busVoltage*INA_BUS_VOLTAGE_LSB/1000;                  // conversion to get milliVolts     //
...
int16_t INA226_Class::getShuntMicroVolts(const bool waitSwitch,               //                                  //
  ...
  shuntVoltage = shuntVoltage*INA_SHUNT_VOLTAGE_LSB/1000;                       // Convert to microvolts            //
...
*******************************************************************************************************************/
int32_t INA226_Class::getBusMilliAmps(const uint8_t deviceNumber) {           //                                  //
  inaDet ina;                                                                 // Hold device details in structure //
  EEPROM.get((deviceNumber%_DeviceCount)*sizeof(ina),ina);                    // Read EEPROM values               //
...
*******************************************************************************************************************/
int32_t INA226_Class::getBusMilliWatts(const uint8_t deviceNumber) {          //                                  //
  inaDet ina;                                                                 // Hold device details in structure //
  EEPROM.get((deviceNumber%_DeviceCount)*sizeof(ina),ina);                    // Read EEPROM values               //
  int32_t milliWatts = readWord(INA_POWER_REGISTER,ina.address);              // Get the raw value                //
          milliWatts = (int64_t)milliWatts*ina.power_LSB/1000;                // Convert to milliwatts            //
  return(milliWatts);                                                         // return computed milliwatts       //
